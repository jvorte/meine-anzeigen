    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 text-gray-900 dark:text-gray-100">
                <h3 class="text-3xl font-bold mb-4">{{ $motorradAd->title }}</h3>

                <p class="text-gray-700 dark:text-gray-300 mb-4">{{ $motorradAd->description }}</p>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        <p class="font-semibold">Preis:</p>
                        <p>{{ $motorradAd->price ?? 'N/A' }} €</p>
                    </div>
                    <div>
                        <p class="font-semibold">Marke:</p>
                        <p>{{ $motorradAd->brand ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="font-semibold">Modell:</p>
                        <p>{{ $motorradAd->model ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="font-semibold">Baujahr:</p>
                        <p>{{ $motorradAd->year ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="font-semibold">Kilometerstand:</p>
                        <p>{{ $motorradAd->mileage ?? 'N/A' }} km</p>
                    </div>
                    <div>
                        <p class="font-semibold">Hubraum:</p>
                        <p>{{ $motorradAd->engine_size ?? 'N/A' }} ccm</p>
                    </div>
                    {{-- Add more motorcycle-specific details here based on your table --}}
                </div>

                {{-- Example for displaying images if you have a relationship --}}
                {{-- Make sure you have an 'images' relationship defined in your MotorradAd model --}}
                    ...
                    <div class="mt-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            @foreach ($motorradAd->images as $image)
                                <img src="{{ asset('storage/' . $image->path) }}" alt="Motorradbild" class="w-full h-48 object-cover rounded-lg shadow-md">
                            @endforeach
                        </div>
                    </div>
                @endif
