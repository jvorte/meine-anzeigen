        ...
        <div class="relative flex flex-col md:flex-row items-center justify-between space-y-4 md:space-y-0 md:space-x-4 p-6 bg-cover bg-center shadow-lg rounded-lg"
            ...
            <div class="relative z-10 w-full md:w-1/2 lg:w-2/5">
                <form action="{{ route('ads.search') }}" method="GET">
                    <input type="text" name="query" placeholder="Was suchst du? z.B. Harley Davidson, Yamaha..."
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-white rounded-lg shadow-md overflow-hidden">
                <div class="px-6 py-5 bg-white dark:bg-gray-100 border-b border-gray-200 dark:border-gray-300">
                    <h3 class="text-4xl font-extrabold text-gray-700 dark:text-gray-800 mb-2 leading-tight">{{ $motorradAd->title }}</h3>
                    <p class="text-2xl font-bold text-indigo-500 dark:text-indigo-600">
                        {{ number_format($motorradAd->price ?? 0, 2, ',', '.') }} €
                    </p>
                    <div class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4">
                        @if (isset($motorradAd->user_id)) {{-- Assuming you have a user_id on your model --}}
                            <a href="{{ route('messages.create', $motorradAd->user_id) }}"
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 mb-6 text-sm">
                        ...
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            ...
                            <span class="text-gray-700 dark:text-gray-800">{{ $motorradAd->brand ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            <span class="font-semibold text-gray-500 dark:text-gray-600">Modell:</span>
                            <span class="text-gray-700 dark:text-gray-800">{{ $motorradAd->model ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            <span class="font-semibold text-gray-500 dark:text-gray-600">Baujahr:</span>
                            <span class="text-gray-700 dark:text-gray-800">{{ $motorradAd->year ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                        ...
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            <span class="font-semibold text-gray-500 dark:text-gray-600">Hubraum:</span>
                            <span class="text-gray-700 dark:text-gray-800">{{ $motorradAd->engine_size ?? 'N/A' }} ccm</span>
                        </div>
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                        ...
                        {{-- Add more motorcycle-specific details here based on your table --}}
                    </div>

                    {{-- Example for displaying images if you have a relationship --}}
                    @if (isset($motorradAd->images) && $motorradAd->images->count() > 0)
                        <div class="mt-6">
