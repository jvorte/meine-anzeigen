...
{{-- ----------------------------------breadcrumbs --------------------------------------------------- --}}
    ...
    <div class="py-2">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <x-breadcrumbs :items="[
        ['label' => 'realEstate Anzeigen', 'url' => route('ads.create')],
        ['label' => 'realEstate Anzeige', 'url' => route('ads.create')],
    ]" />
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-white rounded-lg shadow-md overflow-hidden">
                <div class="px-6 py-5 bg-white dark:bg-gray-100 border-b border-gray-200 dark:border-gray-300">
                    <h3 class="text-4xl font-extrabold text-gray-700 dark:text-gray-800 mb-2 leading-tight">{{ $realEstate->title }}</h3>
                    <p class="text-2xl font-bold text-indigo-500 dark:text-indigo-600">
                        {{ number_format($realEstate->price ?? 0, 2, ',', '.') }} €
                    </p>
                    <div class="flex flex-col sm:flex-row items-center space-y-3 sm:space-y-0 sm:space-x-4">
                        @if (isset($realEstate->user_id)) {{-- Assuming you have a user_id on your model --}}
                            <a href="{{ route('messages.create', $realEstate->user_id) }}"
                ...
                <div class="p-6">
                    <p class="text-gray-600 mb-6 leading-relaxed">{{ $realEstate->description }}</p>

                    <h4 class="text-xl font-semibold text-gray-600 mb-4 border-b pb-2 border-gray-200 dark:border-gray-300">Immobiliendetails</h4>
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-3 gap-x-6 mb-6 text-sm">
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            <span class="font-semibold text-gray-500 dark:text-gray-600">Immobilientyp:</span>
                            <span class="text-gray-700 dark:text-gray-800">{{ $realEstate->property_type ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            <span class="font-semibold text-gray-500 dark:text-gray-600">Adresse:</span>
                            <span class="text-gray-700 dark:text-gray-800">{{ $realEstate->address ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            <span class="font-semibold text-gray-500 dark:text-gray-600">Wohnfläche:</span>
                            <span class="text-gray-700 dark:text-gray-800">{{ $realEstate->area_sqm ?? 'N/A' }} m²</span>
                        </div>
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            <span class="font-semibold text-gray-500 dark:text-gray-600">Anzahl Zimmer:</span>
                            <span class="text-gray-700 dark:text-gray-800">{{ $realEstate->num_rooms ?? 'N/A' }}</span>
                        </div>
                        <div class="flex justify-between items-center bg-gray-50 dark:bg-gray-100 p-3 rounded-md">
                            <span class="font-semibold text-gray-500 dark:text-gray-600">Baujahr:</span>
                            <span class="text-gray-700 dark:text-gray-800">{{ $realEstate->year_built ?? 'N/A' }}</span>
                        </div>
                        ...
                        <div class="mt-6">
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                @foreach ($realEstate->images as $image)
                                    <img src="{{ asset('storage/' . $image->path) }}" alt="Immobilienbild" class="w-full h-48 object-cover rounded-lg shadow-sm">
                                @endforeach
                            </div>
                        </div>
                    @endif
