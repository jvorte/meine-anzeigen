...
<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center px-4 py-2">
            <div>
               ...
               {{ __('Browse all available ads for household items.') }}
                </p>
            </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <div class="p-6 rounded-xl bg-gray-50 border border-gray-200">
                ...
                <div class="flex flex-wrap items-center gap-4 mb-4">
                    ...
                    <div class="flex-grow min-w-[150px]">
                        ...
                        <select name="category" id="category" class="w-full rounded-lg p-1 border-gray-300 shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500">
                ...
                <input type="number" name="min_price" value="{{ request('min_price') }}" placeholder="{{ __('min_price') }}" class="w-full rounded-l-lg border-gray-300 shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500 p-1" />
            </div>
                    <div class="flex-grow min-w-[150px] relative">
                        <label for="max_price" class="sr-only">{{ __('Price: Most expensive first') }}</label>
                        <input type="number" name="max_price" value="{{ request('max_price') }}" placeholder="{{ __('max_price') }}" class="w-full rounded-r-lg border-gray-300 shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500 p-1" />
                    </div>
                ...
                <div x-show="showMoreFilters" x-collapse.duration.300ms class="mt-4 border-t border-gray-200 pt-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        {{-- Brand Dropdown --}}
                        <div class="flex-grow min-w-[150px]">
                            <label for="material" class="block text-sm font-medium text-gray-700">{{ __('brand') }}</label>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 my-5">
            ...
            <a href="{{ route('categories.household.show', $household->id) }}" class="block bg-white shadow rounded-lg overflow-hidden hover:shadow-lg transition duration-300 transform hover:-translate-y-1">
                ...
                <div class="p-4">
                    ...
                    <p class="text-gray-600 mt-1">{{ __('condition') }}: {{ $household->condition ?? 'Ν/Α' }}</p>
                    <p class="text-gray-500 mt-1">{{ __('Category') }}: {{ $household->category ?? 'Ν/Α' }}</p>
                    @if($household->price)
            ...
            @empty
            <p class="text-gray-600 col-span-full">{{ __('No household items were found.') }}</p>
            @endforelse
        </div>
