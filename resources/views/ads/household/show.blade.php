    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg p-6 text-gray-900 dark:text-gray-100">
                <h3 class="text-3xl font-bold mb-4">{{ $householdItem->title }}</h3>

                <p class="text-gray-700 dark:text-gray-300 mb-4">{{ $householdItem->description }}</p>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                    <div>
                        <p class="font-semibold">Preis:</p>
                        <p>{{ $householdItem->price ?? 'N/A' }} €</p>
                    </div>
                    <div>
                        <p class="font-semibold">Zustand:</p>
                        <p>{{ $householdItem->condition ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="font-semibold">Material:</p>
                        <p>{{ $householdItem->material ?? 'N/A' }}</p>
                    </div>
                    <div>
                        <p class="font-semibold">Farbe:</p>
                        <p>{{ $householdItem->color ?? 'N/A' }}</p>
                    </div>
                    {{-- Add more household item-specific details here based on your table --}}
                    {{-- For example: --}}
                    {{-- <div><p class="font-semibold">Abmessungen:</p><p>{{ $householdItem->dimensions ?? 'N/A' }}</p></div> --}}
                    {{-- <div><p class="font-semibold">Alter:</p><p>{{ $householdItem->age ?? 'N/A' }}</p></div> --}}
                </div>

                {{-- Example for displaying images if you have a relationship --}}
                {{-- Make sure you have an 'images' relationship defined in your HouseholdItem model --}}
                    ...
                    <div class="mt-6">
                        ...
                        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                            @foreach ($householdItem->images as $image)
                                <img src="{{ asset('storage/' . $image->path) }}" alt="Haushaltsartikelbild" class="w-full h-48 object-cover rounded-lg shadow-md">
                            @endforeach
                        </div>
                    </div>
                @endif
