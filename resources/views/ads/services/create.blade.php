...
<x-app-layout>
    ...
    <div class="py-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <x-breadcrumbs :items="[
                ['label' => __('Servises Ads'), 'url' => route('categories.services.index')],
                ['label' =>  __('New Servise Ad'), 'url' => route('ads.create')],
            ]" class="mb-8" />
        </div>
    </div>
        ...
        <form method="POST" action="{{ route('ads.services.store') }}" enctype="multipart/form-data" class="space-y-10">
            ...
            <section class="bg-gray-50 p-8 rounded-xl shadow-inner border border-gray-300">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                    ...
                    <div>
                        <label for="service_type" class="block text-sm font-semibold text-gray-700 mb-3">{{ __('Category') }}</label>
                    ...
                    <div>
                        ...
                        <select name="availability" id="availability"
                            class="w-full p-3 border border-gray-300 rounded-lg shadow-sm transition focus:border-blue-500 focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50">
                            <option value="" disabled {{ old('availability') ? '' : 'selected' }}>{{ __('please_select') }}</option>
                            @foreach($availabilityOptions as $availabilityOption)
                            <option value="{{ $availabilityOption }}" {{ old('availability') == $availabilityOption ? 'selected' : '' }}>
                ...
                {{-- Beschreibung --}}
                <div class="mt-8">
                    <label for="description" class="block text-sm font-semibold text-gray-700 mb-3">{{ __('Description of the service') }}</label>
                    <textarea name="description" id="description" rows="7" placeholder="{{ __('Describe your service in detail here. What do you offer? What experience do you have?') }}"
                        class="w-full p-4 border border-gray-300 rounded-lg shadow-sm transition focus:border-blue-500 focus:ring-2 focus:ring-blue-400 focus:ring-opacity-50 resize-none">{{ old('description') }}</textarea>
                    @error('description')
                    <p class="text-red-600 text-sm mt-1 font-semibold">{{ $message }}</p>
                    @enderror
                </div>
                        ...
                        <template x-for="(image, index) in previews" :key="index">
                            <div class="relative group rounded-lg overflow-hidden shadow-lg border border-gray-200 hover:shadow-xl transition">
                                ...
                                <button type="button" @click="remove(index)"
                                    ...
                                    tabindex="0" aria-label="Foto entfernen">✕</button>
                            </div>
                        </template>
                    </div>
                </div>
