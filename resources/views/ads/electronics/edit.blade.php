...
@endif
    ...
    <div class="max-w-6xl mx-auto p-6 bg-white rounded-lg shadow-xl mt-6">
        ...
        <form method="POST" action="{{ route('ads.electronics.update', $electronic->id) }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6" x-data="{
                    ...
                    <div>
                        ...
                        <select name="condition" id="condition"
                            ...
                            @foreach($conditions as $condition)
                                <option value="{{ $condition }}" {{ old('condition', $model->condition ?? '') == $condition ? 'selected' : '' }}>
                                    {{ $condition }}
                                </option>
                    ...
                    <div>
                        ...
                        <select name="warranty_status" id="warranty_status"
                            class="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                            <option value="">Bitte wählen</option>
                            @foreach($garrantyStatus as $status)
                    ...
                    <div x-show="shouldShowField('ram')" x-transition:enter.duration.500ms
                        ...
                        <input type="text" name="ram" id="ram" value="{{ old('ram', $electronic->ram) }}"
                            placeholder="z.B. 8GB"
                            class="w-full p-2 border border-gray-300 rounded-md shadow-sm focus:border-blue-500 focus:ring focus:ring-blue-500 focus:ring-opacity-50">
                        @error('ram')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
    ...
    <div class="mt-4">
        <label class="inline-flex items-center">
            <input 
                ...
                {{ old('show_phone', $electronic->show_phone) ? 'checked' : '' }}
            >
    ...
    <div class="mt-2">
        <label class="inline-flex items-center">
            <input 
                ...
                value="1" 
                class="rounded border-gray-300"
                {{ old('show_mobile_phone', $electronic->show_mobile_phone) ? 'checked' : '' }}
            >
    ...
    <div class="mt-2">
        <label class="inline-flex items-center">
            <input 
                ...
                value="1" 
                class="rounded border-gray-300"
                {{ old('show_email', $electronic->show_email) ? 'checked' : '' }}
            >
            ...
            <section class="bg-gray-50 p-6 rounded-lg shadow-inner">
                ...
                <script>
                    function editImageUploader(initialImages) {
                        return {
                            ...
                            addNewFiles(event) {
                                ...
                                // Update the actual file input to reflect changes in `newFiles` array
                            ...
                            removeNew(index) {
                                ...
                                const fileInput = this.$el.querySelector('#new_images');
                                const dataTransfer = new DataTransfer();
                                this.newFiles.forEach(file => dataTransfer.items.add(file));
