        ...
        <form action="{{ route('ads.boats.index') }}" method="GET" x-data="{ showMoreFilters: false }">
            <div class="p-6 rounded-xl bg-gray-50 border border-gray-200">
                ...
                <div class="flex flex-wrap items-center gap-4 mb-4">
                    ...
                    {{-- Boat Type Dropdown --}}
                    <div class="flex-grow min-w-[150px]">
                        <label for="boat_type" class="sr-only">{{__('boat_type') }}</label>
                        <select name="boat_type" id="boat_type"
                            class="w-full rounded-lg border-gray-300 shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500">
                            <option value="">{{__('boat_type') }}</option>
                ...
                <div x-show="showMoreFilters" x-collapse.duration.300ms class="mt-4 border-t border-gray-200 pt-4">
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            <label for="year_of_construction" class="block text-sm font-medium text-gray-700">{{ __('year_of_construction') }}</label>
                            <select name="year_of_construction" id="year_of_construction"
                                class="mt-1 w-full rounded-lg border-gray-300 shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">{{ __('select_placeholder') }}</option>
                        ...
                        {{-- Sort By Dropdown --}}
                        <div>
                            <label for="sort_by" class="block text-sm font-medium text-gray-700">{{ __('sort_by') }}</label>
                            <select name="sort_by" id="sort_by"
                                class="mt-1 w-full rounded-lg border-gray-300 shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500">
                                <option value="">{{ __('select_placeholder') }}</option>
                                <option value="latest" {{ request('sort_by') == 'latest' ? 'selected' : '' }}>{{__('latest') }}
                                </option>
                                <option value="price_asc" {{ request('sort_by') == 'price_asc' ? 'selected' : '' }}>{{ __('price_asc') }}</option>
                                <option value="price_desc" {{ request('sort_by') == 'price_desc' ? 'selected' : '' }}>{{__('price_desc') }}</option>
                            </select>
                        </div>
                    </div>
                </div>
